...
//if someone wants to submit a comment he must have logged in first. So here we check if they have logged in. Otherwise the log in page will open.
...
if(isset($_POST['comments']))
{
     ...
     if(mysql_query("INSERT INTO VideoComments(title,comment,approved) VALUES('$rtitle','$tcomments ','0')"))
     {
                        //if the submission is successful then we add data to the other columns as well
                        //for every new entry we increase the id by 1
                        mysql_query("UPDATE VideoComments SET id =id + 1");

                        //and we insert the username of the person who submitted this comment
                        mysql_query("UPDATE VideoComments,Users SET VideoComments.user=Users.username WHERE VideoComments.comment='$tcomments' AND Users.user_id=".$_SESSION['user']);
...
<!-- Checking the post method from the page 'tutorial-menu.php'. If it is set then we take the values from the input in the form.-->
<div align="center">
        <?php
              if(isset($_POST['btn-answer']))
              {
	             ...
	             $title = mysql_real_escape_string($_POST['title']);
                     $info  = mysql_real_escape_string($_POST['description']);
        ...
        <br>
        <div class="col-4">
              <video width='100' height='120' controls><source src="<?php echo $path; ?>" type='video/mp4'> </video>
              <br><br><br>
              <p><b>Description:</b></p>
...
<!-- The user can choose if he wants to submit a video and not just a comment, but it'soptional. In the question below ("Do you want to upload video?"), if he clicks 'No' then he will just submit a comment (which will be waiting to be approved by the user), otherwise the method UploadFile() will be called in order to submit the video which will be stored on the server but its information (title, description,path etc) will be inserted into the database.  -->
...
<!-- Here the previous comments submitted by users will be shown. We select only the comments which have been approved by the admin.-->
    <div class="col-8">
              <?php
                        ...
                        {
	                        $title = mysql_real_escape_string($_POST['title']);
	                        //this POST method (tutorial-menu.php) calls this page, we need the title from the question the user chose to read, so we retrieve the 
                                //value from the form in the 'tutorial-menu' page.
              
                        }
                       
                       $res=mysql_query("SELECT * FROM VideoComments WHERE approved='1' AND title='$title'");
                       
                       $count = mysql_num_rows($res);

                     //we count the rows from the table VideoComments . If there are comments, then we show the title below and the columns 'comment' and 'user' 
                       if($count !=0) {
              ?>

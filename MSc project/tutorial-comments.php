...
//The admin is the only one who has access to this page. He checks the video comments, submitted by users, for every tutorial and he can either approve them // or delete them from the database. If the comments are approved then it will be shown in the page. They weren't visible before.
...
if(isset($_POST['delete-comment']))
{
 ...
 //when we delete a row, we also need to check that the id from the remaining rows will change as well. For example, if the table comments has 7 rows           //and we delete row 5, then the rows 6 & 7 will change to 5 & 6.
      $c = mysql_query("SELECT id FROM VideoComments WHERE VideoComments.id  > '$id' ORDER BY VideoComments.id DESC");
 ...
 //if the command DELETE is successful, then we change the id for every row with id>del_id as we mentioned above.
      if(mysql_query("DELETE FROM VideoComments WHERE VideoComments.title = '$title' AND VideoComments.comment='$comment'")){
            
            $count = mysql_num_rows($c); 

            for($i=0; $i<$count; $i++){
                   
                   mysql_query("UPDATE VideoComments SET VideoComments.id =". $t++ . " WHERE VideoComments.id  > '$id' ORDER BY VideoComments.id DESC");
...
if(isset($_POST['approve-comment']))
{
   ...
   mysql_query("UPDATE VideoComments SET VideoComments.approved = 1 WHERE VideoComments.title = '$title' AND VideoComments.comment='$comment'");
}
?>
...
<div class="col-12">
    <?php
        ...
        $query = "SELECT * FROM VideoComments ORDER BY VideoComments.id ASC";
        ...
        if(mysql_num_rows($result)>0) {
                  // output data of each row
                   while($row = mysql_fetch_assoc($result)) {
                            //output data from table VideoComments.
                            if ($row["approved"] != "1" ) {
